...
const DEFAULT_STATS: ProblemStat[] = [
...
interface ProblemProps {
  eyebrow?: string;
  headline?: RichText;
  /** Right-column paragraphs (LP pages). Omit → homepage paragraphs. */
  body?: RichText[];
  handwritten?: string;
  stats?: ProblemStat[];
}
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8 relative">
        <div className="reveal max-w-[1100px]">
          ...
          <h2 className="font-display tracking-display mt-4 text-[40px] md:text-[72px] lg:text-[88px] leading-[0.98] text-gomu-purple-deep">
            {headline ? (
              renderRich(headline, { onDark: false })
            ) : (
              <>
              ...
              <div className="relative grid grid-cols-3 grid-rows-2 gap-4 h-full">
                {PILLS.map((lbl) => (
                  ...
                  >
                    ...
                    <div className="font-display italic text-[14px] md:text-[17px]">{lbl}</div>
          ...
          <div className="reveal">
            {body ? (
              body.map((para, i) => (
                <p
                  key={i}
                  className="text-[17px] md:text-[19px] leading-[1.6] text-gomu-ink/90 font-display [&+p]:mt-5"
                >
                  {renderRich(para, { onDark: false })}
                </p>
              ))
            ) : (
              <>
                <p className="text-[18px] md:text-[20px] leading-[1.6] text-gomu-ink/90 font-display">
                  ...
                  <b className="text-gomu-purple-deep not-italic font-sans">5 pots différents</b>. 60 à
                  100€ par mois. 15 gélules à orchestrer chaque matin.
                </p>
                <p className="mt-5 text-[16px] md:text-[17px] leading-[1.65] text-gomu-ink/80">
                  ...
                  Ce n&apos;est pas un problème de volonté — c&apos;est un modèle économique conçu pour
                  te faire <i className="font-display">acheter plus</i> et prendre moins.
                </p>
              </>
            )}
            <p className="mt-6 handwritten text-[22px] text-gomu-purple-1">{handwritten}</p>
          </div>
        </div>
